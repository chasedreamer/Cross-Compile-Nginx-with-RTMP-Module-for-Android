...
ngx_init_cycle(ngx_cycle_t *old_cycle)
{
    ...
    printf("old_cycle->conf_param=%s\n",old_cycle->conf_param.data);
    if (cycle->conf_param.data == NULL) {
        ngx_destroy_pool(pool);
        return (ngx_cycle_t*)105;//NULL;
    }
    ...
    if (conf.args == NULL) {
        ngx_destroy_pool(pool);
        return (ngx_cycle_t*)116;//NULL;
    }

    conf.temp_pool = ngx_create_pool(NGX_CYCLE_POOL_SIZE, log);
    if (conf.temp_pool == NULL) {
        ngx_destroy_pool(pool);
        return (ngx_cycle_t*)117;//NULL;
    }
    ...
    if (ngx_conf_param(&conf) != NGX_CONF_OK) {
        ...
        return (ngx_cycle_t*)118;//NULL;
    }

    if (ngx_conf_parse(&conf, &cycle->conf_file) != NGX_CONF_OK) {
        environ = senv;
        ngx_destroy_cycle_pools(&conf);
        return (ngx_cycle_t*)119;//NULL;
    }
    ...
    if (ngx_test_lockfile(cycle->lock_file.data, log) != NGX_OK) {
             return (ngx_cycle_t*)125;
        goto failed;
    }


    if (ngx_create_paths(cycle, ccf->user) != NGX_OK) {
             return (ngx_cycle_t*)126;
        goto failed;
    }


    if (ngx_log_open_default(cycle) != NGX_OK) {
             return (ngx_cycle_t*)127;
        goto failed;
    }
    ...
    for (i = 0; /* void */ ; i++) {
        ...
        if (ngx_shm_alloc(&shm_zone[i].shm) != NGX_OK) {
             return (ngx_cycle_t*)132;
            goto failed;
        }

        if (ngx_init_zone_pool(cycle, &shm_zone[i]) != NGX_OK) {
             return (ngx_cycle_t*)133;
            goto failed;
        }

        if (shm_zone[i].init(&shm_zone[i], NULL) != NGX_OK) {
             return (ngx_cycle_t*)134;
            goto failed;
        }
